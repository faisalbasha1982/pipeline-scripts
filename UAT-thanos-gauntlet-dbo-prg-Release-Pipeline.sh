...
ls -ahl
cd thanos-gauntlet-u-dbo-prg/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python/env-db-prg.txt'
...
helm upgrade thanos-gauntlet-u-dbo-prg thanos-gauntlet-u-dbo-prg -n uat \
...
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-u-dbo-prg/values.yaml \
