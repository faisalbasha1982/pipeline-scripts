...
ls -ahl
cd thanos-gauntlet-dbo-rcsbtr/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python-dev/env-db-rcsbtr.txt'
...
helm upgrade thanos-gauntlet-dbo-rcsbtr thanos-gauntlet-dbo-rcsbtr -n development \
--set 'imagePullSecrets[0].name=regcred' \
--set replicaCount=$(replicaCount) \
--set group_id='$(group_id)' \
--set kafka_topic_prefix='$(kafka_topic_prefix)' \
--set image.repository=docker-registry.osn.com/thanos-gauntlet \
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-dbo-rcsbtr/values.yaml \
