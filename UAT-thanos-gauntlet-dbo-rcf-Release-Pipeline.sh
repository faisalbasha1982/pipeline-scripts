...
ls -ahl
cd thanos-gauntlet-u-dbo-rcf/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python/env-db-rcf.txt'
...
helm upgrade thanos-gauntlet-u-dbo-rcf thanos-gauntlet-u-dbo-rcf -n uat \
...
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-u-dbo-rcf/values.yaml \
