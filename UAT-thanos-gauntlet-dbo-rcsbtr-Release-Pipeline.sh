...
ls -ahl
cd thanos-gauntlet-u-dbo-rcsbtr/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python/env-db-rcsbtr.txt'
...
helm upgrade thanos-gauntlet-u-dbo-rcsbtr thanos-gauntlet-u-dbo-rcsbtr -n uat \
...
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-u-dbo-rcsbtr/values.yaml \
