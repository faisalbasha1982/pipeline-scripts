...
ls -ahl
cd thanos-gauntlet-u-dbo-tap/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python/env-db-tap.txt'
...
helm upgrade thanos-gauntlet-u-dbo-tap thanos-gauntlet-u-dbo-tap -n uat \
...
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-u-dbo-tap/values.yaml \
