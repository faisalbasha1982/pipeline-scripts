...
ls -ahl
cd thanos-gauntlet-u-dbo-srs/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python/env-db-srs.txt'
...
helm upgrade thanos-gauntlet-u-dbo-srs thanos-gauntlet-u-dbo-srs -n uat \
...
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-u-dbo-srs/values.yaml \
