...
ls -ahl
cd thanos-gauntlet-dbo-srs/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python-dev/env-db-srs.txt'
...
helm upgrade thanos-gauntlet-dbo-srs thanos-gauntlet-dbo-srs -n development \
--set 'imagePullSecrets[0].name=regcred' \
--set replicaCount=1 \
--set group_id='$(group_id)' \
--set kafka_topic_prefix='$(kafka_topic_prefix)' \
--set image.repository=docker-registry.osn.com/thanos-gauntlet \
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-dbo-srs/values.yaml \
