...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/devadmin/env-python/env-db-rpu.txt'
...
helm upgrade thanos-gauntlet-dbo-rpu thanos-gauntlet-dbo-rpu -n production \
--set 'imagePullSecrets[0].name=regcred' \
--set replicaCount=$(replicaCount) \
--set group_id='$(group_id)' \
--set kafka_topic_prefix='$(kafka_topic_prefix)' \
--set image.repository=cicd-dev01.osn.com/thanos-gauntlet-prod \
--set image.tag=$(Build.BuildId) -f thanos-gauntlet-dbo-rpu/values.yaml \
