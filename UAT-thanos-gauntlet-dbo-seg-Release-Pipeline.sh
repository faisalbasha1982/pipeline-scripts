...
ls -ahl
cd thanos-gauntlet-u-dbo-seg/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python/env-db-seg.txt'
...
helm upgrade thanos-gauntlet-u-dbo-seg thanos-gauntlet-u-dbo-seg -n uat \
...
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-u-dbo-seg/values.yaml \
