...
ls -ahl
cd thanos-gauntlet-dbo-dll/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python-dev/env-db-dll.txt'
...
helm upgrade thanos-gauntlet-dbo-dll thanos-gauntlet-dbo-dll -n development \
--set 'imagePullSecrets[0].name=regcred' \
--set replicaCount=1 \
--set group_id='thanosSyncDEV25022021' \
--set kafka_topic_prefix='bcm_uat_thanos_gauntlet' \
--set image.repository=docker-registry.osn.com/thanos-gauntlet \
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-dbo-dll/values.yaml \
