...
ls -ahl
cd thanos-gauntlet-dbo-rcf/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python-dev/env-db-rcf.txt'
...
helm upgrade thanos-gauntlet-dbo-rcf thanos-gauntlet-dbo-rcf -n development \
--set 'imagePullSecrets[0].name=regcred' \
--set replicaCount=$(replicaCount) \
--set group_id='$(group_id)' \
--set kafka_topic_prefix='$(kafka_topic_prefix)' \
--set image.repository=docker-registry.osn.com/thanos-gauntlet \
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-dbo-rcf/values.yaml \
