...
ls -ahl
cd thanos-gauntlet-u-dbo-lic/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python/env-db-lic.txt'
...
helm upgrade thanos-gauntlet-u-dbo-lic thanos-gauntlet-u-dbo-lic -n uat \
...
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-u-dbo-lic/values.yaml \
