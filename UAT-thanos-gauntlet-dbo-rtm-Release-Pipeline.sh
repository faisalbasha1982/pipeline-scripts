...
ls -ahl
cd thanos-gauntlet-u-dbo-rtm/templates
...
do
  ...
  if [ "$n" -eq 35 ]
  then
       envfile='/home/dotnet_user/env-python/env-db-rtm.txt'
...
helm upgrade thanos-gauntlet-u-dbo-rtm thanos-gauntlet-u-dbo-rtm -n uat \
...
--set image.tag=$(Build.BuildNumber) -f thanos-gauntlet-u-dbo-rtm/values.yaml \
